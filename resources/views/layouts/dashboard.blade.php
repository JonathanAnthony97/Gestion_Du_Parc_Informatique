...
<head>
	...
	<title>Bolloré | Transport & Logistic </title>
	<link href="{{ asset('bootstrap/css/bootstrap.min.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/main.min.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/hider.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/plugins.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/responsive.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/icons.min.css') }}" rel="stylesheet" type="text/css" />
	<link href="{{ asset('assets/css/fichier_css.css') }}" rel="stylesheet" type="text/css">
	<link href="{{ asset('assets/css/anime.css') }}" rel="stylesheet" type="text/css">
	...
	<link href="{{ asset('alert/x0popup.min.css') }}" rel="stylesheet">
</head>
<body>
	<header class="header navbar navbar-fixed-top" role="banner">
		<div class="container">
			...
			<a class="navbar-brand" href="#">
				<img id="logo" src="{{ asset('assets/image/logo.png') }}" alt="logo" />
			</a>
			...
			<ul class="nav navbar-nav navbar-left hidden-xs hidden-sm">
				<li class="dropdown">
					...
					<ul id="MenuC" class="dropdown-menu">
					</ul>
				</li>
				<li>
					<a href="{{ route('suivi') }}">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu">
						<li><a href="home"><i class="icon-refresh"></i> Reparation</a></li>
						<li><a href="home"><i class="icon-cogs"></i> Maintenance</a></li>
					</ul>
				</li>
				<li>
					<a href="{{ route('invent') }}">
			...
			<div class="modal-dialog" style="width:400px;margin-top: 50px;">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title"><i class="icon-edit"></i>&nbsp; Etats</h4>
					</div>
							...
							<div class="row">
								<div class="col-md-10 col-md-offset-1">
									...
									<div class="input-group" style="margin-bottom: 5px;">
										<select id="etat" class="select2 col-md-12">
											
										</select>
										<span class="input-group-addon btn btn-xs changeEtat">Modifier</span>
									</div>
								</div>
						</div>
					</div>
...
<script type="text/javascript" src="{{ asset('assets/js/libs/jquery-1.10.2.min.js') }}"></script>
	...
	<script type="text/javascript" src="{{ asset('assets/js/bootstrap-datetimepicker.fr.js') }}"></script>
	<script type="text/javascript" src="{{ asset('plugins/bootbox/bootbox.js') }}"></script>
	<script src="{{ asset('alert/x0popup.min.js') }}"></script>
	<script type="text/javascript" src="{{ asset('assets/js/app.js') }}"></script>
	<script type="text/javascript" src="{{ asset('assets/js/plugins.js') }}"></script>
	<script type="text/javascript" src="{{ asset('assets/js/plugins.form-components.js') }}"></script>
	<script type="text/javascript" src="{{ asset('assets/js/hideScrollbar.js') }}"></script>
	<script type="text/javascript" src="{{ asset('assets/js/demo/pages_calendar.js')}}"></script>
	<script type="text/javascript" src="{{ asset('chart/VChart.min.js')}}"></script>
	<script type="text/javascript" src="{{ asset('chart/MyChart.js')}}"></script>
	<script>
	$(document).ready(function(){
		...
		var par_page14 = "{{ Session::get('Par_page14') }}";
		var par_page15 = "{{ Session::get('Par_page15') }}";
		...
		$('#const14').find('option[value="'+par_page14+'"]').attr('selected',true);
		$('#const15').find('option[value="'+par_page15+'"]').attr('selected',true);
		...
		$('#s2id_const14 .select2-choice > .select2-chosen').html(par_page14);
		$('#s2id_const15 .select2-choice > .select2-chosen').html(par_page15);
		}
		...
		function initEtat(){
			$.ajax({
				...
				success:function(data){
					$('#s2id_etat .select2-choice > .select2-chosen').html("Selectionner");
					$('#etat').append('<option value="0">Selectionner</option>');
					$.each(data,function(index,ind){
						$('#etat').append('<option value="'+ind.id_eta+'">'+ind.etat+'</option>');
					});
				}
			});	
		}

		$(document).on('click','.changeEtat',function(){
			if($('#etat').val() != '0'){
				var tab = [];
				$('span[class="checked dlt"]').each(function(i){
		 		...
		 		$.ajax({
		 			type:'get',
		 			url :'setEtat?idEta='+$('#etat').val()+'&tab='+tab,
		 			success:function(data){
		...
		$(document).on('click','tbody tr #togler3',function(){
		 ...
		 $(document).on('click','.change_etat',function(){
		 	var tab = [];
		 	$('span[class="checked dlt"]').each(function(i){
		 			tab[i] = $(this).find('#checker').data('id');
		 		});
		 ...
		 $(document).on('click','#typer',function(e){
		 	e.preventDefault();
		 		...
		 		if(categ == "Ordinateur"){
		 			...
		 			$('label[for="l"]').html('Nom (Netbios) :');
		 			$('input[id="l"]').attr('name','netbios');
		 			$('.ethernet').css('display','none');
		 			$('.console').css('display','none');
		 		}
		 		if(categ == "Electrique"){
		 			$('.variant').css('display','block');
		 			$('.intensite').css('display','block');
		 			$('label[for="l"]').html('Puissance (Kva) :');
		 			$('input[id="l"]').attr('name','puissance');
		 			$('.ethernet').css('display','none');
		 			$('.console').css('display','none');
		 		}
		 		if(categ == "Peripherique"){
					...
					success:function(data){
						if(data.errors){
							...
							$('#m[data-row="'+data.id_ma+'"]').replaceWith('<tr id="m" data-row="'+data.id_ma+'">'+
											...
											'<td>'+formateur2(data.date_acqui)+'</td>'+
											'<td>'+data.nom+'</td>'+
											'<td><i id="i_eta" style="color: #4d77cc" class="icon-sign-blank"></i>&nbsp; En service</td>'+
											'<td>'+data.vlr_acqui+'</td>'+
											'<td class="align-center"><span class="btn-group">'+
												'<a id="sub" data-id="'+data.id_ma+'" data-marque="'+data.marque+'" data-serie="'+data.num_serie+'" class="btn btn-xs" href="#">'+
													'<i class="icon-th-list"></i>'+
				...
				success:function(data){
					if(data.errors){
						...
						$('#m[data-row="'+data.id_ma+'"]').replaceWith('<tr id="m" data-row="'+data.id_ma+'">'+
											...
											'<td>'+formateur2(data.date_acqui)+'</td>'+
											'<td>'+data.nom+'</td>'+
											'<td><i id="i_eta" style="color: #e25856" class="icon-sign-blank"></i>&nbsp; '+data.etat+'</td>'+
											'<td>'+data.vlr_acqui+'</td>'+
											'<td class="align-center"><span class="btn-group">'+
												'<a id="sub" data-id="'+data.id_ma+'" data-marque="'+data.marque+'" data-serie="'+data.num_serie+'" class="btn btn-xs" href="#">'+
													'<i class="icon-th-list"></i>'+
...
//modificztion d'une panne
	...
	$(document).on('click','a[href="#tabe_f3"]',function(){
		...
		//histopanne
		$(document).on('click','.pagin_histopane .pagination a',function(e){
			e.preventDefault();
			var arg = $('input[name="search_histopan"]').val();
			var page = $(this).attr('href').split('page=')[1];
			$('#processeur20').css('visibility','visible');
			$.ajax({
				type : 'get',
				url : 'histopane?page='+page+'&arg='+arg,
				success:function(data){
					$('#ajax_histopan').html(data);
					$('#processeur20').css('visibility','hidden');
				}
			});
		});

		$(document).on('click','.pagin_detailPane .pagination a',function(e){
			e.preventDefault();
			var arg = $('input[name="search_detailpan"]').val();
			var page = $(this).attr('href').split('page=')[1];
			var idMa = $('tr[class="activated"]').data('id');
			$('#processeur21').css('visibility','visible');
			$.ajax({
				type : 'get',
				url :'histopanAjax?page='+page+'&id='+idMa+'&arg='+arg,
				success:function(data){
					$('#ajax_listPane').html(data);
					$('#processeur21').css('visibility','hidden');
				}
			});
		});

		$(document).on('change','#const14',function(){
			$('input[name="search_histopan"]').val("");
		$('#processeur20').css('visibility','visible');
			$.ajax({
				type:'get',
				url :'histopane/'+$(this).val(),
				success:function(data){
					$('#ajax_histopan').html(data);
					$('#processeur20').css('visibility','hidden');
				}
		...
		$(document).on('keyup','input[name="search_histopan"]',function(){
			$('#processeur20').css('visibility','visible');
			$.ajax({
				type:'get',
				url:'histopane?arg='+$(this).val(),
				success:function(data){
					$('#ajax_histopan').html(data);
					$('#processeur20').css('visibility','hidden');
				}
		...
		$(document).on('change','#const15',function(){
			$('#processeur21').css('visibility','visible');
			$('input[name="search_detailpan"]').val("");
			var idMa = $('tr[class="activated"]').data('id');
			$.ajax({
				type:'get',
				url:'histopanAjax/'+$(this).val()+'?id='+idMa,
				success:function(data){
					$('#ajax_listPane').html(data);
					$('#processeur21').css('visibility','hidden');
				}
		...
		$(document).on('keyup','input[name="search_detailpan"]',function(){
		
			var idMa = $('tr[class="activated"]').data('id');
			$('#processeur21').css('visibility','visible');
			$.ajax({
				type:'get',
				url: 'histopanAjax?id='+idMa+'&arg='+$(this).val(),
				success:function(data){
					$('#ajax_listPane').html(data);
					$('#processeur21').css('visibility','hidden');
				}
		...
		function ReactualiseHistoPane(idMa){
			$.ajax({
				type:'get',
				url:'histopane',
				success:function(data){
					$('#ajax_histopan').html(data);
					$('tr[data-id="'+idMa+'"]').css('background-color','#ececec');
					$('tr[data-id="'+idMa+'"]').addClass('activated');
				}
			});
		}

		$(document).on('click','#DeletePane',function(){
			var id = $(this).data('id');
				...
				if(data.button == 'warning'){
					...
					$.ajax({
						...
						success:function(data){
							$.ajax({
								type:'get',
								url: 'histopanAjax?id='+idMa,
								success:function(data){
								$('#ajax_listPane').html(data);
								}
							});
							ReactualiseHistoPane(idMa);
							x0p('suppresion','reussie !','ok',false);
						}
					});
		...
		$(document).on('click','#TotalPaneExcel',function(){
			window.location.replace('TotalPaneExcel/xlsx');
		});

		$(document).on('click','#HistoPaneExcel',function(){
			var target = $('tr[class="activated"]');
			var idMa = $('tr[class="activated"]').data('id');
			if(idMa != undefined){
				var type = target.find('#type').data('type');
				var seri = target.find('#serie').data('serie');
				var marque = target.find('#marque').data('marque');
				window.location.replace('HistoPaneExcel/xlsx?idMa='+idMa+'&type='+type+'&seri='+seri+'&marque='+marque);
			}else{
				x0p('Message','Aucune information trouvée !','info',false);
			}
		});

		$(document).on('click','#histoPane',function(){
			$('#processeur21').css('visibility','visible');
				$('tr[id="tr_colorable"]').css('background-color','white');
				$('tr[id="tr_colorable"]').removeClass('activated');
				var target = $(this).parent().parent().parent();
				target.css('background-color','#ececec');
				if(target.hasClass('activated')){
					target.removeClass('activated');
				}else{
					target.addClass('activated');
					var dep = target.find('#dep').data('dep');
					var type = target.find('#type').data('type');
					var seri = target.find('#serie').data('serie');
					var marque = target.find('#marque').data('marque');
					var pane = target.find('#pane').data('pane');
					$('#cont1').html(dep);
					$('#cont2').html(seri);
					$('#cont3').html(marque);
					$('#cont4').html(type);
		...
		function activation(){
			var id = $('#tr_colorable').data('id');
			$('#histoPane[data-id="'+id+'"]').trigger('click');
		}
		activation();
		...
		$(document).on('click','#TracabilitExcel',function(e){
		e.preventDefault();
			var type = $('#traceType').val();
			var arg = $('input[name="search_trace"]').val();
			window.location.replace('TracabilitExcel/xlsx?type='+type+'&arg='+arg);
		});

		$(document).on('click','#InterventionExcel',function(e){
			e.preventDefault();
			var type = $('#interType').val();
			var arg = $('input[name="search_histo"]').val();
			window.location.replace('InterventionExcel/xlsx?type='+type+'&arg='+arg);
		});

		$(document).on('click','.bilanExcel',function(e){
			e.preventDefault();
			var date = $('input[name="keyDate"]').val();
			window.location.replace('BilanExcel/xlsx?date='+date);
		});

		$(document).on('click','#TracExcel',function(e){
			e.preventDefault();
			var mat = $('.titl').val();
			var serie = $('input[name="temp_serie"]').val();
			window.location.replace('TracExcel/xlsx?serie='+serie+'&mat='+mat);
		});

		$(document).on('click','#MaintExcel',function(e){
			e.preventDefault();
			var mat = $('.titl').val();
			var idm = $('input[name="temp_id"]').val();
			window.location.replace('MaintExcel/xlsx?idMa='+idm+'&mat='+mat);
		});

		$(document).on('click','#ReparExcel',function(e){
			e.preventDefault();
			var mat = $('.titl').val();
			var idm = $('input[name="temp_id"]').val();
			window.location.replace('ReparExcel/xlsx?idRep='+idm+'&mat='+mat);
		});

		$(document).on('click','#PanExcel',function(e){
			e.preventDefault();
			var mat = $('.titl').val();
			var idm = $('input[name="temp_id"]').val();
			window.location.replace('PanExcel/xlsx?idMa='+idm+'&mat='+mat);
		});
